...
class ApiView:
    ...
    def put(self, request=None, package=None):
        data = request.json if package is None else package
        try:
            entry = self.instance.query.get(data.get(self.id, None))

            if entry is not None:
                for i in data:
                    if hasattr(entry, i) and i != self.id:
                        setattr(entry, i, data.get(i, None))

                self.db.session.commit()

                new_instance = self.parse_model(self.instance.query.get(data.get(self.id, None)))
    ...
    def post(self, request=None, package=None):
        ...
        try:
            ...
            self.db.session.commit()
            if self.on_data_change is not None:
                self.on_data_change(self.parse_model(entry), 'post')
            return jsonify(self.parse_model(entry)), 201
        except SQLAlchemyError as e:
            return jsonify({'status': 'error', 'description': str(e), 'code': 400}), 400

    def get(self, request):
        try:
            entry = self.instance.query.get(request.args.get(self.id))
            if entry is not None:
                return jsonify(self.parse_model(entry)), 200
            else:
                return jsonify({'status': 'error', 'description': 'not_found', 'code': 404}), 404
        except SQLAlchemyError as e:
            return jsonify({'status': 'error', 'description': str(e), 'code': 400}), 400
    ...
    def delete(self, request):
        try:
            entry = self.instance.query.get(request.json.get(self.id))
            if entry is not None:
                self.db.session.delete(entry)
                self.db.session.commit()
                if self.on_data_change is not None:
                    self.on_data_change({self.id: request.json.get(self.id)}, 'delete')
    ...
    def list(self, request, base_query=[]):
        data = request.args
        if data is None:
            data = dict()
        ...
        for i in query:
            response.append(self.parse_model(i))
